 ...
 * TODO: Conectar con /api/alumnos cuando esté lista la BD.
 */
...
import { MOCK_ALUMNOS, CICLOS, CURSOS } from "@/shared/mockData";
...
export default function AlumnosPage() {
  ...
  const handleGuardar = () => {
    if (!form.nombre || !form.nia || !form.ciclo || !form.curso) return alert("Rellena los campos obligatorios");
    // TODO: POST /api/alumnos
    setAlumnos(prev => [{ id: Date.now(), ...form }, ...prev]);
    setForm(EMPTY);
  };

  const handleEliminar = (id: number) => {
    if (!confirm("¿Eliminar este alumno?")) return;
    // TODO: DELETE /api/alumnos/:id
    setAlumnos(prev => prev.filter(a => a.id !== id));
  };

  return (
    <div>
      <PageHeader breadcrumb="Inicio" breadcrumbHighlight="/ Alumnos" title="Gestión de Alumnos" subtitle="Alta de alumnos en prácticas y consulta del censo por ciclo y curso." />
      ...
      <Card className="mb-7">
        ...
        <div className="p-6">
          <FormRow cols={2}>
            <FormGroup label="Nombre completo *"><input className={INPUT_CLS} value={form.nombre} onChange={e => set("nombre")(e.target.value)} placeholder="Nombre y apellidos" /></FormGroup>
            <FormGroup label="NIA *"><input className={INPUT_CLS} value={form.nia} onChange={e => set("nia")(e.target.value)} placeholder="Número de identificación" /></FormGroup>
          </FormRow>
          <FormRow cols={2}>
            <FormGroup label="Teléfono *"><input className={INPUT_CLS} value={form.telefono} onChange={e => set("telefono")(e.target.value)} placeholder="6XX XXX XXX" /></FormGroup>
            <FormGroup label="Correo electrónico *"><input className={INPUT_CLS} type="email" value={form.email} onChange={e => set("email")(e.target.value)} placeholder="alumno@educa.gva.es" /></FormGroup>
          </FormRow>
          <FormRow cols={2}>
